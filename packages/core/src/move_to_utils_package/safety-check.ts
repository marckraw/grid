...
export const createSafetyGuardrail = () => {
  const safetyGuardrailModel = openai("gpt-4.1");
  const refuserModel = openai("gpt-4.1-mini");

  const checkIsSafe = async (history: string): Promise<GuardrailResult> => {
    const { object } = await generateObject({
      model: safetyGuardrailModel,
      schema: z.object({
  ...
  const refuse = (safetyGuardrailResult: GuardrailResult) => {
    return streamText({
      model: refuserModel,
      system:
        "You are a content safety guardrail. Refuse to answer unsafe questions.",
      prompt:
        safetyGuardrailResult.reason ||
...
export const safetyGuardrail = createSafetyGuardrail();
