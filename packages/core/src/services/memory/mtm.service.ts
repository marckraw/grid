  ...
  const extractFactsFromEvents = (
    ...
    events.forEach((event) => {
      if (event.type.includes("message") && event.data.message && !nameFound) {
        ...
        const explicitNamePatterns = [/my name is (\w+)/i, /call me (\w+)/i];
    ...
    if (!nameFound) {
      events.forEach((event) => {
        if (
          ...
          const implicitPatterns = [/i'?m (\w+)/i, /i am (\w+)/i];
...
Return only valid JSON, no explanations.`;
        ...
        messages: [
          {
            role: "system",
            content:
              "You are a fact extraction assistant. Extract facts from conversations and return JSON.",
          },
  ...
  const generateMarkdownSummary = (
    ...
    if (
      summary.extractedFacts.userName ||
      summary.extractedFacts.userPreferences?.length
    ) {
      ...
      Object.entries(sessions)
        ...
        .forEach(([sessionId, events]) => {
          ...
          events.slice(0, 6).forEach((event) => {
            const role = event.type.includes("user")
              ? "**User**"
              : "**Assistant**";
  ...
  const searchFacts = async (query: string): Promise<MTMSummary[]> => {
    ...
    for (const dateStr of summaryDates) {
      ...
      if (summary) {
        ...
        if (
          factsStr.includes(queryLower) ||
          highlightsStr.includes(queryLower)
        ) {
