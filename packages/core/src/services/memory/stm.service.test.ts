...
describe("createSimpleSTMService", () => {
  ...
  describe("log", () => {
    it("should create directory if it does not exist", async () => {
      ...
      expect(mockFs.mkdir).toHaveBeenCalledWith("./memory", {
        recursive: true,
      });
    });
    ...
    it("should append event with timestamp to log file", async () => {
      ...
      expect(new Date(writtenEvent.timestamp).toISOString()).toBe(
        writtenEvent.timestamp,
      );
    });
  ...
  describe("getByType", () => {
    ...
    it("should respect limit parameter", async () => {
      const stm = createSimpleSTMService();
      const events: MemoryEvent[] = Array(10)
        .fill(null)
        .map((_, i) => ({
          timestamp: new Date().toISOString(),
          type: "test.event",
    ...
    it("should use default limit of 100", async () => {
      const stm = createSimpleSTMService();
      const events: MemoryEvent[] = Array(150)
        .fill(null)
        .map((_, i) => ({
          timestamp: new Date().toISOString(),
          type: "test.event",
