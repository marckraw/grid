...
import type { STMService, MTMService, MemoryEvent, MTMSummary } from '../services/memory/memory.types.js';
...
describe('createMemoryTools', () => {
  ...
  describe('searchRecentMemory', () => {
    ...
    it('should filter by event type', async () => {
      ...
      expect(result.filteredEvents).toBe(2);
      expect(result.events).toHaveLength(2);
      expect(result.events.every(e => e.type === 'conversation.user.message')).toBe(true);
    });
    ...
    it('should limit results to last 10 events', async () => {
      const tools = createMemoryTools({ stm: mockSTM });
      const testEvents: MemoryEvent[] = Array(20).fill(null).map((_, i) => ({
        timestamp: new Date().toISOString(),
        type: 'test.event',
        data: { index: i }
      }));
  ...
  describe('recallConversationHistory', () => {
    it('should recall both user and agent messages', async () => {
      ...
      expect(mockSTM.getByType).toHaveBeenCalledWith('conversation.user.message', 10);
      expect(mockSTM.getByType).toHaveBeenCalledWith('conversation.agent.response', 10);
    ...
    it('should recall only user messages when specified', async () => {
      ...
      expect(mockSTM.getByType).toHaveBeenCalledWith('conversation.user.message', 5);
  ...
  describe('getMemoryStatistics', () => {
    it('should calculate statistics from events', async () => {
      ...
      const testEvents: MemoryEvent[] = [
        {
          timestamp: new Date(now.getFullYear(), now.getMonth(), now.getDate(), hour1).toISOString(),
          type: 'conversation.user.message',
          data: {},
          metadata: { source: 'cli' }
        },
        ...
        {
          timestamp: new Date(now.getFullYear(), now.getMonth(), now.getDate(), hour2).toISOString(),
          type: 'conversation.agent.response',
          data: {},
      ...
      const totalActivity = Object.values(result.activityByHour).reduce((sum, count) => sum + count, 0);
  ...
  describe('recallFacts', () => {
    it('should recall facts from MTM summaries', async () => {
      ...
      expect(result.foundFacts.userPreferences).toContain('favorite color: blue');
      expect(result.foundFacts.userPreferences).toContain('favorite food: pizza');
