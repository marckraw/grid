...
export const dataConverterTool = createNamedTool({
  ...
  inputSchema: z.object({
    ...
    options: z.object({
      headers: z.boolean().optional().default(true).describe("Include headers in CSV/TSV output"),
  ...
  execute: async ({ from, to, data, options = {} }) => {
    try {
      ...
      if (from === to) {
        return { result: data, message: "Source and target formats are the same" };
      }
      ...
      switch (to) {
        ...
        case "csv":
          result = toDelimited(parsedData, options.delimiter || ",", options.headers ?? true);
          break;
...
function toDelimited(data: any[], delimiter: string, includeHeaders: boolean): string {
