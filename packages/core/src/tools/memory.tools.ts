...
export const createMemoryTools = (deps: {
  ...
  const searchRecentMemory = createNamedTool({
    ...
    description:
      "Search through recent memory events from the last N hours. Useful for recalling recent conversations, events, or activities.",
  ...
  const recallConversationHistory = createNamedTool({
    ...
    description:
      'Recall previous messages from this or recent conversations. Useful when user refers to "earlier", "before", "what we discussed".',
    inputSchema: z.object({
      limit: z
        .number()
        .default(10)
  ...
  const getMemoryStatistics = createNamedTool({
    ...
    description:
      "Get statistics about memory usage, event types, and patterns.",
    inputSchema: z.object({
      hours: z.number().default(24).describe("Time window for statistics"),
    }),
    execute: async ({ hours }) => {
      const events = await stm.getRecent(hours);
      ...
      return {
        ...
        mostActiveHour:
          Object.entries(eventsByHour).sort(([, a], [, b]) => b - a)[0]?.[0] ||
          "none",
        mostCommonType:
          Object.entries(eventsByType).sort(([, a], [, b]) => b - a)[0]?.[0] ||
          "none",
      };
    },
  });
  ...
  const searchMemoryByTags = createNamedTool({
    ...
    description:
      "Search memory events by their tags. Useful for finding related events across different categories.",
    inputSchema: z.object({
      tags: z
        .array(z.string())
  ...
  const recallFacts = createNamedTool({
    name: "recall_facts",
    description:
      "Recall important facts like user name, preferences, and key information from memory summaries.",
    inputSchema: z.object({
      query: z
        .string()
        .describe(
 ...
 */
export const getMemoryToolsArray = (deps: {
  stm: STMService;
  mtm?: MTMService;
}) => {
