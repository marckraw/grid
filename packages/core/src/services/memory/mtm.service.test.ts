...
import type { STMService, MemoryEvent, MTMSummary, LLMService } from './memory.types.js';
...
describe('createMTMService', () => {
  ...
  describe('summarizeDay', () => {
    ...
    it('should create directory if it does not exist', async () => {
      ...
      expect(mockFs.mkdir).toHaveBeenCalledWith(defaultStoragePath, { recursive: true });
    });
    ...
    it('should generate summary with pattern-based fact extraction', async () => {
      ...
      expect(summary.extractedFacts.userPreferences).toContain('favorite color: blue and I love cats');
  ...
  describe('summarizeDay with LLM', () => {
    ...
    const testEvents: MemoryEvent[] = [
      ...
      },
      {
        timestamp: new Date('2024-01-15T10:01:00Z').toISOString(),
        type: 'conversation.agent.response',
        data: { message: 'Nice to meet you Bob! How can I help with your engineering work?' }
      }
    ];

    it('should use LLM for fact extraction when available', async () => {
      const mtm = createMTMService({ stm: mockSTM, llmService: mockLLMService });
      mockSTM.getRecent = vi.fn().mockResolvedValue(testEvents);
      ...
      expect(summary.extractedFacts.keyTopics).toContain('software engineering');
    });
  ...
  describe('getSummary', () => {
    it('should retrieve existing summary', async () => {
      ...
      const mockSummary: MTMSummary = {
        ...
        conversations: { count: 1, totalMessages: 5, avgLength: 50, topics: [] },
        eventStatistics: {},
      ...
      expect(mockFs.readFile).toHaveBeenCalledWith('./memory/mtm/2024-01-15.json', 'utf-8');
  ...
  describe('getSummaryMarkdown', () => {
    it('should retrieve existing markdown summary', async () => {
      const mtm = createMTMService({ stm: mockSTM });
      const testDate = new Date('2024-01-15');
      const mockMarkdown = '# Daily Memory Summary - 2024-01-15\n\nTest content';

      mockFs.readFile.mockResolvedValueOnce(mockMarkdown);

      const markdown = await mtm.getSummaryMarkdown(testDate);

      expect(mockFs.readFile).toHaveBeenCalledWith('./memory/mtm/2024-01-15.md', 'utf-8');
  ...
  describe('searchFacts', () => {
    it('should search across all summaries', async () => {
      ...
      mockFs.readdir.mockResolvedValueOnce(['2024-01-14.json', '2024-01-15.json']);
      ...
      const summary2: MTMSummary = {
        ...
        conversations: { count: 1, totalMessages: 3, avgLength: 40, topics: [] },
        eventStatistics: {},
  ...
  describe('markdown generation', () => {
    it('should generate comprehensive markdown with all sections', async () => {
      ...
      const markdownCall = mockFs.writeFile.mock.calls.find(
        call => call[0].includes('.md')
      );
      ...
      for (let i = 0; i < 5; i++) {
        for (let j = 0; j < 10; j++) {
          multiSessionEvents.push({
            timestamp: new Date(`2024-01-15T${10 + i}:${String(j).padStart(2, '0')}:00Z`).toISOString(),
            type: j % 2 === 0 ? 'conversation.user.message' : 'conversation.agent.response',
